...
expName = 'Mooney Face Localization'  # from the Builder filename that created this script
expInfo = {'participant': '', 'Student ID number (this is for RPP purpose only)': '', 'Age': '', 'Gender': '', 'Handedness': '', 'In this experiment, you need to constantly adjust the position of your cursor. what tool are your using to complete this experiment (mouse, trackpad,etc)?': '', 'Do you have normal or correct to normal vision?': ''}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\jerry\\Downloads\\mooney-face-localization-master\\mooney-face-localization-master\\facelocalization_lastrun.py',
...
F = visual.ImageStim(
    ...
    flipHoriz=False, flipVert=False,
    texRes=128, interpolate=True, depth=0.0)
mouse_gender_task = event.Mouse(win=win)
x, y = [None, None]
mouse_gender_task.mouseClock = core.Clock()
M = visual.ImageStim(
    win=win,
    name='M', 
    image='M.png', mask=None,
    ori=0, pos=(0.15, 0), size=(0.1, 0.1),
    color=[1,1,1], colorSpace='rgb', opacity=1,
    flipHoriz=False, flipVert=False,
    texRes=128, interpolate=True, depth=-2.0)
fix_gender_task = visual.ImageStim(
    win=win,
    name='fix_gender_task', 
    image='fixation.png', mask=None,
    ori=0, pos=(0, 0), size=(0.2, 0.2),
    color=[1,1,1], colorSpace='rgb', opacity=1,
    flipHoriz=False, flipVert=False,
...
ConsentComponents = [consent, agree, disagree, mouse_5]
...
while continueRoutine:
    ...
    # *consent* updates
    if consent.status == NOT_STARTED and frameN >= 0.0:
    ...
    # *agree* updates
    if agree.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
    ...
    # *disagree* updates
    if disagree.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
    ...
    # *mouse_5* updates
    if mouse_5.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
...
thisExp.addData('agree.started', agree.tStartRefresh)
thisExp.addData('agree.stopped', agree.tStopRefresh)
thisExp.addData('disagree.started', disagree.tStartRefresh)
thisExp.addData('disagree.stopped', disagree.tStopRefresh)
...
# keep track of which components have finished
WelcomeComponents = [text, key_resp_11]
...
while continueRoutine:
    ...
    # *text* updates
    if text.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
    ...
    # *key_resp_11* updates
    waitOnFlip = False
    if key_resp_11.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
...
while continueRoutine:
    ...
    # *key_resp* updates
    waitOnFlip = False
    if key_resp.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
...
_key_resp_2_allKeys = []
# keep track of which components have finished
Intro_2Components = [text_3, key_resp_2]
...
while continueRoutine:
    ...
    # *text_3* updates
    if text_3.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
    ...
    # *key_resp_2* updates
    waitOnFlip = False
    if key_resp_2.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
...
_key_resp_3_allKeys = []
# keep track of which components have finished
Intro_3Components = [text_4, key_resp_3]
...
while continueRoutine:
    ...
    # *text_4* updates
    if text_4.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
    ...
    # *key_resp_3* updates
    waitOnFlip = False
    if key_resp_3.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
...
# keep track of which components have finished
Intro_4Components = [text_7, key_resp_10]
...
while continueRoutine:
    ...
    # *text_7* updates
    if text_7.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
    ...
    # *key_resp_10* updates
    waitOnFlip = False
    if key_resp_10.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
...
_key_resp_4_allKeys = []
# keep track of which components have finished
CheckComponents = [text_5, key_resp_4]
...
while continueRoutine:
    ...
    # *text_5* updates
    if text_5.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
    ...
    # *key_resp_4* updates
    waitOnFlip = False
    if key_resp_4.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
...
_key_resp_8_allKeys = []
# keep track of which components have finished
DemoComponents = [text_11, key_resp_8]
...
while continueRoutine:
    ...
    # *text_11* updates
    if text_11.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
    ...
    # *key_resp_8* updates
    waitOnFlip = False
    if key_resp_8.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
...
demo = data.TrialHandler(nReps=1, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('testTrialLoop.xlsx'),
    seed=None, name='demo')
...
for thisDemo in demo:
    ...
    gotValidClick = False  # until a click is received
    # keep track of which components have finished
    exampleComponents = [image_2, mouse_2, image_5]
    ...
    while continueRoutine:
        ...
        # *mouse_2* updates
        if mouse_2.status == NOT_STARTED and frameN >= 60:
            # keep track of start time/frame for later
    ...
    for thisComponent in exampleComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    demo.addData('image_2.started', image_2.tStartRefresh)
    demo.addData('image_2.stopped', image_2.tStopRefresh)
    # store data for demo (TrialHandler)
    if len(mouse_2.x): demo.addData('mouse_2.x', mouse_2.x[0])
    if len(mouse_2.y): demo.addData('mouse_2.y', mouse_2.y[0])
    if len(mouse_2.leftButton): demo.addData('mouse_2.leftButton', mouse_2.leftButton[0])
    if len(mouse_2.midButton): demo.addData('mouse_2.midButton', mouse_2.midButton[0])
    if len(mouse_2.rightButton): demo.addData('mouse_2.rightButton', mouse_2.rightButton[0])
    if len(mouse_2.time): demo.addData('mouse_2.time', mouse_2.time[0])
    demo.addData('mouse_2.started', mouse_2.tStart)
    demo.addData('mouse_2.stopped', mouse_2.tStop)
    demo.addData('image_5.started', image_5.tStartRefresh)
    demo.addData('image_5.stopped', image_5.tStopRefresh)
    ...
    gotValidClick = False  # until a click is received
    # keep track of which components have finished
    backtocenterComponents = [F, mouse_gender_task, M, fix_gender_task]
    for thisComponent in backtocenterComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    backtocenterClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    
    # -------Run Routine "backtocenter"-------
    while continueRoutine:
        # get current time
        t = backtocenterClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=backtocenterClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        
        # *F* updates
        if F.status == NOT_STARTED and frameN >= 0.0:
            # keep track of start time/frame for later
        ...
        if mouse_gender_task.status == NOT_STARTED and frameN >= 0.0:
            # keep track of start time/frame for later
            mouse_gender_task.frameNStart = frameN  # exact frame index
            mouse_gender_task.tStart = t  # local t and not account for scr refresh
            mouse_gender_task.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(mouse_gender_task, 'tStartRefresh')  # time at next scr refresh
            mouse_gender_task.status = STARTED
        ...
        if mouse_gender_task.status == STARTED:  # only update if started and not finished!
            buttons = mouse_gender_task.getPressed()
            if buttons != prevButtonState:  # button state changed?
                prevButtonState = buttons
                if sum(buttons) > 0:  # state changed to a new click
                    # check if the mouse was inside our 'clickable' objects
                    ...
                    for obj in [F,M]:
                        if obj.contains(mouse_gender_task):
                            gotValidClick = True
                            mouse_gender_task.clicked_name.append(obj.name)
                    x, y = mouse_gender_task.getPos()
                    mouse_gender_task.x.append(x)
                    mouse_gender_task.y.append(y)
                    buttons = mouse_gender_task.getPressed()
                    mouse_gender_task.leftButton.append(buttons[0])
                    mouse_gender_task.midButton.append(buttons[1])
                    mouse_gender_task.rightButton.append(buttons[2])
                    mouse_gender_task.time.append(mouse_gender_task.mouseClock.getTime())
                    if gotValidClick:  # abort routine on response
                        continueRoutine = False
        ...
        if fix_gender_task.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            fix_gender_task.frameNStart = frameN  # exact frame index
            fix_gender_task.tStart = t  # local t and not account for scr refresh
            fix_gender_task.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(fix_gender_task, 'tStartRefresh')  # time at next scr refresh
    ...
    for thisComponent in backtocenterComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    demo.addData('F.started', F.tStartRefresh)
    demo.addData('F.stopped', F.tStopRefresh)
    # store data for demo (TrialHandler)
    if len(mouse_gender_task.x): demo.addData('mouse_gender_task.x', mouse_gender_task.x[0])
    if len(mouse_gender_task.y): demo.addData('mouse_gender_task.y', mouse_gender_task.y[0])
    if len(mouse_gender_task.leftButton): demo.addData('mouse_gender_task.leftButton', mouse_gender_task.leftButton[0])
    if len(mouse_gender_task.midButton): demo.addData('mouse_gender_task.midButton', mouse_gender_task.midButton[0])
    if len(mouse_gender_task.rightButton): demo.addData('mouse_gender_task.rightButton', mouse_gender_task.rightButton[0])
    if len(mouse_gender_task.time): demo.addData('mouse_gender_task.time', mouse_gender_task.time[0])
    if len(mouse_gender_task.clicked_name): demo.addData('mouse_gender_task.clicked_name', mouse_gender_task.clicked_name[0])
    demo.addData('mouse_gender_task.started', mouse_gender_task.tStart)
    demo.addData('mouse_gender_task.stopped', mouse_gender_task.tStop)
    demo.addData('M.started', M.tStartRefresh)
    demo.addData('M.stopped', M.tStopRefresh)
    demo.addData('fix_gender_task.started', fix_gender_task.tStartRefresh)
    demo.addData('fix_gender_task.stopped', fix_gender_task.tStopRefresh)
...
_key_resp_6_allKeys = []
# keep track of which components have finished
startComponents = [text_8, key_resp_6]
...
while continueRoutine:
    ...
    # *key_resp_6* updates
    waitOnFlip = False
    if key_resp_6.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
...
trials = data.TrialHandler(nReps=2, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('trialLoop.xlsx'),
...
for thisTrial in trials:
    ...
    gotValidClick = False  # until a click is received
    # keep track of which components have finished
    trialComponents = [image, mouse, image_6]
    ...
    while continueRoutine:
        ...
        # *mouse* updates
        if mouse.status == NOT_STARTED and frameN >= 60:
            # keep track of start time/frame for later
        ...
        # *image_6* updates
        if image_6.status == NOT_STARTED and frameN >= 0.0:
            # keep track of start time/frame for later
    ...
    for thisComponent in backtocenterComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials.addData('F.started', F.tStartRefresh)
    trials.addData('F.stopped', F.tStopRefresh)
    # store data for trials (TrialHandler)
    if len(mouse_gender_task.x): trials.addData('mouse_gender_task.x', mouse_gender_task.x[0])
    if len(mouse_gender_task.y): trials.addData('mouse_gender_task.y', mouse_gender_task.y[0])
    if len(mouse_gender_task.leftButton): trials.addData('mouse_gender_task.leftButton', mouse_gender_task.leftButton[0])
    if len(mouse_gender_task.midButton): trials.addData('mouse_gender_task.midButton', mouse_gender_task.midButton[0])
    if len(mouse_gender_task.rightButton): trials.addData('mouse_gender_task.rightButton', mouse_gender_task.rightButton[0])
    if len(mouse_gender_task.time): trials.addData('mouse_gender_task.time', mouse_gender_task.time[0])
    if len(mouse_gender_task.clicked_name): trials.addData('mouse_gender_task.clicked_name', mouse_gender_task.clicked_name[0])
    trials.addData('mouse_gender_task.started', mouse_gender_task.tStart)
    trials.addData('mouse_gender_task.stopped', mouse_gender_task.tStop)
    trials.addData('M.started', M.tStartRefresh)
    trials.addData('M.stopped', M.tStopRefresh)
    trials.addData('fix_gender_task.started', fix_gender_task.tStartRefresh)
    trials.addData('fix_gender_task.stopped', fix_gender_task.tStopRefresh)
    # the Routine "backtocenter" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    _key_resp_5_allKeys = []
    # keep track of which components have finished
    Pause_1Components = [text_6, key_resp_5]
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    Pause_1Clock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    ...
    while continueRoutine:
        ...
        tThisFlip = win.getFutureFlipTime(clock=Pause_1Clock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        
        # *text_6* updates
        if text_6.status == NOT_STARTED and frameN >= 0.0:
            # keep track of start time/frame for later
        ...
        # *key_resp_5* updates
        waitOnFlip = False
        if key_resp_5.status == NOT_STARTED and frameN >= 0.0:
            # keep track of start time/frame for later
...
_key_resp_7_allKeys = []
# keep track of which components have finished
EndComponents = [text_10, key_resp_7]
...
while continueRoutine:
    ...
    # *text_10* updates
    if text_10.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
    ...
    # *key_resp_7* updates
    waitOnFlip = False
    if key_resp_7.status == NOT_STARTED and frameN >= 0.0:
        # keep track of start time/frame for later
